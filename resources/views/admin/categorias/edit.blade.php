...
<div class="flex flex-col gap-6 max-w-3xl mx-auto">
    <x-admin.page-header
        title="Editar Categoria de Notícia"
        :breadcrumbs="[
    ...
    <form action="{{ route('admin.categorias.update', $categoria->id) }}" method="POST" class="bg-white border border-slate-200 shadow-sm rounded-xl p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="pt-2">
            <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" name="ativo" value="1" class="sr-only peer" {{ $categoria->ativo ? 'checked' : '' }}>
        ...
        <div class="pt-6 border-t border-slate-100 flex justify-end">
            <x-admin.button color="amber" class="px-6 py-2.5 rounded-xl shadow-md flex items-center gap-2">
                <i class="fa-solid fa-check" aria-hidden="true"></i> Atualizar Categoria
            </x-admin.button>
        </div>
    </form>
</div>
