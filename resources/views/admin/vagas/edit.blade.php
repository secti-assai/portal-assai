...
@section('title', 'Editar Oportunidade - Painel Admin')
...
<div class="flex flex-col max-w-4xl gap-6 mx-auto">
    ...
    @endphp
    <x-admin.page-header title="Editar Oportunidade" :breadcrumbs="$breadcrumbsEdit">
        <x-slot:action>
    ...
    <form action="{{ route('admin.vagas.update', $vaga->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 space-y-6 bg-white border shadow-sm border-slate-200 rounded-xl">
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div class="md:col-span-2">
                    ...
                    <input id="titulo" type="text" name="titulo" value="{{ old('titulo', $vaga->titulo) }}" required
                ...
                <div>
                    ...
                    <select id="tipo" name="tipo" required
                        ...
                        <option value="formal" {{ old('tipo', $vaga->tipo) == 'formal' ? 'selected' : '' }}>Vaga Formal (Prefeitura / Empresas)</option>
                        <option value="informal" {{ old('tipo', $vaga->tipo) == 'informal' ? 'selected' : '' }}>Trabalho Informal (Bicos / Autônomos)</option>
                    </select>
                ...
                <div>
                    ...
                    <input id="setor_ou_contato" type="text" name="setor_ou_contato" value="{{ old('setor_ou_contato', $vaga->setor_ou_contato) }}" required
                ...
                <div>
                    ...
                    <input id="data_limite" type="date" name="data_limite"
                        value="{{ old('data_limite', $vaga->data_limite ? $vaga->data_limite->format('Y-m-d') : '') }}"
                ...
                <div>
                    ...
                    <input id="link_acao" type="text" name="link_acao" value="{{ old('link_acao', $vaga->link_acao) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="descricao" name="descricao" rows="4"
                        ...
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all resize-none @error('descricao') border-red-400 bg-red-50 @enderror">{{ old('descricao', $vaga->descricao) }}</textarea>
                ...
                <div class="md:col-span-2">
                    <label class="flex items-center gap-3 p-3 transition border cursor-pointer border-slate-200 rounded-xl hover:bg-slate-50 w-full">
                        <input type="checkbox" name="status" value="1" {{ old('status', $vaga->status) ? 'checked' : '' }}
