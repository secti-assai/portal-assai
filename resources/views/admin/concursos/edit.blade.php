...
<div class="flex flex-col max-w-4xl gap-6 mx-auto">
    ...
    <form action="{{ route('admin.concursos.update', $concurso->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 space-y-6 bg-white border shadow-sm border-slate-200 rounded-xl">
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <input type="text" name="titulo" value="{{ old('titulo', $concurso->titulo) }}" required placeholder="Ex: Concurso Público 01/2025" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                </div>

                <div>
                    <label class="block mb-1.5 text-sm font-bold text-slate-700">Descrição Breve <span class="text-red-500">*</span></label>
                    <textarea name="descricao" rows="4" required placeholder="Descreva os cargos e informações principais..." class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">{{ old('descricao', $concurso->descricao) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="url" name="link" value="{{ old('link', $concurso->link) }}" placeholder="https://..." class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                ...
                <div class="flex items-center">
                    <label class="flex items-center gap-3 p-3 transition border cursor-pointer border-slate-200 rounded-xl hover:bg-slate-50 w-full">
                        <input type="checkbox" name="ativo" value="1" {{ old('ativo', $concurso->ativo) ? 'checked' : '' }} class="w-5 h-5 border-gray-300 rounded text-blue-600 focus:ring-blue-500">
                        <div>
