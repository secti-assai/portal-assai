            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="icone" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition-all">
                        <option value="padrao" {{ old('icone', $servico->icone) == 'padrao' ? 'selected' : '' }}>Padrão (Genérico)</option>
                        <option value="saude" {{ old('icone', $servico->icone) == 'saude' ? 'selected' : '' }}>Saúde (Coração/Cruz)</option>
                        <option value="vagas" {{ old('icone', $servico->icone) == 'vagas' ? 'selected' : '' }}>Vagas/Emprego (Maleta)</option>
                        <option value="documentos" {{ old('icone', $servico->icone) == 'documentos' ? 'selected' : '' }}>Documentos/Notas (Papel)</option>
                        <option value="ouvidoria" {{ old('icone', $servico->icone) == 'ouvidoria' ? 'selected' : '' }}>Ouvidoria (Megafone/Chat)</option>
                        <option value="alvara" {{ old('icone', $servico->icone) == 'alvara' ? 'selected' : '' }}>Alvará/Empresa (Prédio)</option>
                        <option value="educacao" {{ old('icone', $servico->icone) == 'educacao' ? 'selected' : '' }}>Educação (Capelo/Livro)</option>
                    </select>
                </div>
