...
<div class="flex flex-col max-w-4xl gap-6 mx-auto">
    ...
    <form action="{{ route('admin.telefones.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="p-6 space-y-6 bg-white border shadow-sm border-slate-200 rounded-xl">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nome" value="{{ old('nome') }}" required placeholder="Ex: Recepção da Prefeitura" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all">
                </div>

                <div>
                    <label class="block mb-1.5 text-sm font-bold text-slate-700">Número do Telefone <span class="text-red-500">*</span></label>
                    <input type="text" name="numero" value="{{ old('numero') }}" required placeholder="(43) 3262-XXXX" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all">
                </div>
                ...
                <div class="md:col-span-2">
                    <label class="flex items-center gap-3 p-3 transition border cursor-pointer border-slate-200 rounded-xl hover:bg-slate-50 w-full">
                        <input type="checkbox" name="ativo" value="1" {{ old('ativo', '1') == '1' ? 'checked' : '' }} class="w-5 h-5 border-gray-300 rounded text-emerald-600 focus:ring-emerald-500">
                        <div>
