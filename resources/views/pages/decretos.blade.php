...
<section class="pt-24 pb-12 bg-slate-50 min-h-screen"
    x-data="{ 
        ...
        updateList() {
            ...
            setTimeout(() => {
                fetch(`{{ route('decretos.index') }}?${params.toString()}`, {
                    headers: { 'X-Requested-With': 'XMLHttpRequest' }
                })
                .then(res => res.text())
                .then(html => {
                    document.getElementById('list-container').innerHTML = html;
                    window.history.pushState({}, '', `{{ route('decretos.index') }}?${params.toString()}`);
                })
                .finally(() => { 
        ...
        <div class="flex flex-col items-center">
            <div class="relative">
                ...
                <div class="absolute inset-0 flex items-center justify-center">
                    <i class="fa-solid fa-gavel text-blue-600 animate-pulse"></i>
                </div>
            </div>
            <p class="mt-4 text-blue-900 font-extrabold tracking-tight animate-pulse uppercase text-xs">Atualizando Decretos...</p>
        </div>
    </div>
    ...
    <div class="container mx-auto px-4">
        ...
        <div class="mb-10">
            ...
            <p class="text-slate-500">Acesse os decretos municipais oficiais da Prefeitura de Assaí.</p>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-8 relative">
            <form @submit.prevent="updateList()" class="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
                <div class="md:col-span-2">
                    <label for="q" class="block text-sm font-bold text-slate-700 mb-2">Pesquisar</label>
                    <input type="text" x-model="q" placeholder="Número ou súmula..." 
                ...
                <div class="flex gap-2">
                    ...
                    <button type="button" @click="clearFilters()" x-show="q || data_inicio || data_fim || tipo" x-cloak
        ...
        {{-- Listagem Container --}}
        <div id="list-container" class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden min-h-[400px]">
             @include('pages.partials.decretos-list')
        </div>
