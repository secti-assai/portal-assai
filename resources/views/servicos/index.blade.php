...
@section('content')
    ...
    <section class="bg-blue-900 py-12 md:py-16">
        <div class="container px-4 mx-auto max-w-6xl">
            ...
            <h1 class="text-3xl md:text-4xl font-black text-white leading-tight mb-2">Serviços ao Cidadão</h1>
            <p class="text-blue-200 text-base mb-8 max-w-xl">Encontre rapidamente os serviços digitais e presenciais oferecidos pela Prefeitura de Assaí.</p>

            {{-- Barra de busca --}}
            <form method="GET" action="{{ route('servicos.index') }}" class="flex gap-2 max-w-xl">
                ...
                <button type="submit"
                        class="px-5 py-3 text-sm font-bold text-blue-900 bg-yellow-400 rounded-xl hover:bg-yellow-300 transition shrink-0">
                    Buscar
                </button>
            </form>
    ...
    <div class="container px-4 mx-auto max-w-6xl py-10">
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <aside class="lg:col-span-1">
                <div class="sticky top-8 bg-white border border-slate-100 rounded-2xl shadow-sm p-6">
                    <h2 class="text-xs font-black uppercase tracking-widest text-slate-400 mb-4">Filtrar por Secretaria</h2>

                    <ul class="space-y-1 text-sm">
                        <li>
                            <a href="{{ route('servicos.index', request()->only('q')) }}"
                               class="flex items-center gap-2 px-3 py-2 rounded-lg transition
                                      {{ !request('secretaria') ? 'bg-blue-50 font-bold text-blue-700' : 'text-slate-600 hover:bg-slate-50' }}">
                        ...
                        <li>
                            <a href="{{ route('servicos.index', array_merge(request()->only('q'), ['secretaria' => $sec->id])) }}"
                               class="flex items-center gap-2 px-3 py-2 rounded-lg transition
                                      {{ request('secretaria') == $sec->id ? 'bg-blue-50 font-bold text-blue-700' : 'text-slate-600 hover:bg-slate-50' }}">
            ...
            <section class="lg:col-span-3">
                ...
                <div class="flex items-center justify-between mb-6">
                    <p class="text-sm text-slate-500">
                        ...
                        @if(request('q'))
                            para <span class="font-semibold text-blue-700">"{{ request('q') }}"</span>
                        @endif
                    </p>
                    @if(request('q') || request('secretaria'))
                        <a href="{{ route('servicos.index') }}" class="text-xs font-semibold text-slate-400 hover:text-red-500 transition flex items-center gap-1">
                ...
                @if($servicos->isEmpty())
                    ...
                    <div class="flex flex-col items-center justify-center py-20 text-center bg-white border-2 border-dashed border-slate-200 rounded-2xl">
                        ...
                        <p class="text-sm text-slate-400 max-w-xs">Tente outros termos ou remova os filtros ativos.</p>
                ...
                @else
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach($servicos as $servico)
                            <div class="flex flex-col h-full bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-md transition">
                                <div class="flex flex-col flex-1 p-6">
                                    ...
                                    {{-- Secretaria badge --}}
                                    <span class="text-xs font-semibold text-slate-400 mb-1 uppercase tracking-wide">
                                        {{ $servico->secretaria->nome ?? 'Administração Geral' }}
                                    </span>
