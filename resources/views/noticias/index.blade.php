...
@section('content')
<main id="conteudo-principal" accesskey="1" tabindex="-1" class="relative flex flex-col min-h-screen pt-[96px] lg:pt-[160px] pb-16 bg-white" 
    x-data="{ 
        loading: false,
        async navigate(url) {
    ...
    <div class="container max-w-6xl px-4 mx-auto font-sans">
        ...
        <div class="mb-12" id="filters-container" @click.prevent="if($event.target.closest('a')) navigate($event.target.closest('a').href)">
            <div class="flex items-center gap-2 overflow-x-auto flex-nowrap pb-2 pr-2 scrollbar-hide">
                ...
                <a href="{{ route('noticias.index', array_filter(['categoria' => $cat->id, 'q' => request('q')])) }}"
                    class="shrink-0 px-5 py-2 text-xs font-bold rounded-full transition-all whitespace-nowrap border
                   {{ (request('categoria') == $cat->id || request('categoria') == $cat->nome) ? 'bg-blue-800 text-white border-blue-800 shadow-md' : 'bg-white text-gray-600 border-gray-200 hover:border-blue-300 hover:text-blue-700' }}">
                    {{ $cat->nome }}
                </a>
                @endforeach
            </div>
                ...
                <article class="relative h-[350px] md:h-[430px] overflow-hidden rounded-2xl group bg-slate-100 shadow-md hover:shadow-xl transition-shadow duration-300">
                    <a href="{{ route('noticias.show', $noticia->slug) }}" class="block w-full h-full outline-none focus-within:ring-4 focus-within:ring-blue-500 focus-within:ring-inset">
                        ...
                        <div class="absolute bottom-0 left-0 w-full p-5 lg:p-6 bg-white/10 backdrop-blur-lg border-t border-white/30 flex flex-col justify-end transition-all duration-500">
                            ...
                            <span class="inline-block px-2 py-0.5 mb-2 bg-yellow-400 text-blue-900 text-[10px] font-black rounded uppercase tracking-tighter w-fit shadow-sm">
        ...
        {{-- Paginação --}}
        <div class="mt-12" id="pagination-container" @click.prevent="if($event.target.closest('a')) navigate($event.target.closest('a').href)">
